...
public class Registrarse extends JFrame {
    ...
    public Registrarse() {
        ...
        lblTituloRegistro.setBounds(266, 30, 137, 25);
        contentPane.add(lblTituloRegistro);
        
        JLabel lblNombre = new JLabel("Nombre");
        lblNombre.setFont(new Font("Arial Black", Font.PLAIN, 15));
        lblNombre.setBounds(244, 78, 71, 26);
        contentPane.add(lblNombre);

        txtNombre = new JTextField();
        txtNombre.setBounds(351, 78, 113, 24);
        contentPane.add(txtNombre);
        
        JLabel lblCorreo = new JLabel("Correo electrónico");
        lblCorreo.setFont(new Font("Arial Black", Font.PLAIN, 15));
        lblCorreo.setBounds(180, 115, 160, 26);
        contentPane.add(lblCorreo);

        txtCorreo = new JTextField();
        txtCorreo.setBounds(351, 117, 113, 24);
        contentPane.add(txtCorreo);

        JLabel lblTelefono = new JLabel("Teléfono");
        lblTelefono.setFont(new Font("Arial Black", Font.PLAIN, 15));
        lblTelefono.setBounds(229, 155, 98, 26);
        contentPane.add(lblTelefono);

        txtTelefono = new JTextField();
        txtTelefono.setBounds(351, 157, 113, 24);
        contentPane.add(txtTelefono);

        JLabel lblContrasena = new JLabel("Contraseña");
        lblContrasena.setFont(new Font("Arial Black", Font.PLAIN, 15));
        lblContrasena.setBounds(217, 201, 106, 26);
        contentPane.add(lblContrasena);

        txtContrasena = new JPasswordField();
        txtContrasena.setBounds(351, 204, 113, 24);
        contentPane.add(txtContrasena);

        // Botón de registro
        JButton btnRegistrarse = new JButton("Registrarse");
        btnRegistrarse.setBounds(264, 257, 150, 30);
